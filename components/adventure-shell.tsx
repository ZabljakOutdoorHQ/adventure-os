  ...
  return (
    <main className="app-shell">
      <aside className="primary-sidebar">
        ...
        <nav className="space-y-1" aria-label="Primary navigation">
          {navItems.map(({ key, label, icon: Icon }) => (
            <button className={`nav-button ${activeView === key ? "nav-button-active" : ""}`} key={key} onClick={() => setActiveView(key)} type="button">
              <Icon size={17} />{label}
            </button>
          ))}
        </nav>
      ...
      <section className="main-workspace">
        <header className="workspace-header">
          ...
          <div className="relative">
            <label className="search-box"><Search size={16} /><input aria-label="Search Adventure OS" onChange={(event) => setQuery(event.target.value)} placeholder="Search people, projects, groups…" value={query} /></label>
            {searchMatches.length > 0 && <div className="search-results">{searchMatches.map((item) => <button key={`${item.type}-${item.label}`} type="button"><span>{item.type}</span>{item.label}</button>)}</div>}
          </div>
        </header>
      ...
      <aside className="ai-sidebar">
        ...
        <div className="rounded-2xl bg-[var(--background)] p-4 text-sm leading-6">Ask across projects without losing your current position in <strong>{activeLabel}</strong>.</div>
        <div className="mt-4 rounded-2xl border p-4 text-sm"><div className="mb-2 flex items-center gap-2"><AlertTriangle size={15} /><strong>Agent boundary</strong></div><p className="m-0 text-[var(--muted-foreground)]">It may search, summarise and propose. It cannot send, delete, pay or publish.</p></div>
        <div className="mt-auto"><textarea aria-label="Ask Adventure OS" placeholder="What needs my attention today?" /><button className="primary-action" type="button">Ask Adventure OS</button></div>
      </aside>
    </main>
  );
}
